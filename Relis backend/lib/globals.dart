...
String pageName(pageType type){
  switch(type){
    case pageType.category: return "in this Genre";
    case pageType.trending: return "in Trending";
    case pageType.favourite: return "in Favourites";
    case pageType.wishList: return "in WishList";
    case pageType.history: return "in History";
    case pageType.bought: return "Bought";
    case pageType.rented: return "Rented";
    case pageType.personalBooks: return "in your Personal Library";
    case pageType.none:
    default:
      return "";
  }
...
Map<String, dynamic> book1 = {
  ...
  "description": "The Wings of Fire is an autobiography of former Indian President APJ Abdul Kalam. The book covers his life before he became the President of India and commanded the armed forces. Renowned scientist and former Indian President APJ Abdul Kalam from 2002 to 2007 is well known across India and abroad as well.",
  "publication": "XYZ",
  "feedback" : {
...
Map<String, dynamic> book2 = {
  ...
  "description": "This biography by Water Isaacson was published shortly after Jobs death in 2011. Isaacson has the accolade of being Jobs' exclusive official biographer and was granted more than 40 interviews with Jobs and interviewed more than 100 of Jobs colleagues, family members and friends.",
  "publication": "ABC",
  "feedback" : {
...
Map<String, dynamic> book3 = {
  ...
  "description": "One the greatest Indian warriors, Maharana Pratap was born on May 9 in the year 1540. Maharana Pratap was the ruler of Mewar, a province in modern day Rajasthan. Renowned as a fearsome warrior and an excellent combat strategist, Pratap protected the Mewar region against repeated onslaughts from the Mughals.",
  "publication": "PQR",
  "feedback" : {
    "user2user2@relix.app" : {
      "userId" : "user2user2@relix.app",
      "comment": "One the greatest Indian warriors, Maharana Pratap was born on May 9 in the year 1540. Maharana Pratap was the ruler of Mewar, a province in modern day Rajasthan. Renowned as a fearsome warrior and an excellent combat strategist, Pratap protected the Mewar region against repeated onslaughts from the Mughals.",
      "rating": "3",
    },
...
var categoryList = [categoryNovel, categoryThriller, categoryFictional, categorySciFic, categoryHistorical, categoryPoetry, categoryRomance, categoryOther];
...
void loadEachHover() {
  ...
  loadHover(user["recommendedBook"].length, user["recommendedBook"], recommendationHover, "recommendedBook");
  ...
  loadHoverMap(user["booksBought"], boughtHover,);
  loadHoverMap(user["booksRented"], rentedHover,);
  loadHover(user["personalBooks"].length, user["personalBooks"], personalBooksHover, "personalBooks");
}

getHover(pageType type, var hover, var currentBooks) {
    print("cccccccccccccccccccccccccc");
    print("currentBook: ${currentBooks}");
    switch(type) {
      case pageType.category:
        loadHoverMap(category, hover);
      break;
      case pageType.trending:
        loadHover(getBooksMap(user["trendingBook"]).size, getBooksMap(user["trendingBook"]), hover, "trendingBook");
      break;
      case pageType.favourite:
        loadHover(currentBooks.size, currentBooks, hover, "favouriteBook");
      break;
      case pageType.wishList:
        loadHover(currentBooks.size, currentBooks, hover, "wishListBook");
      break;
      case pageType.history:
        loadHover(getBooksMap(user["bookHistory"]).size, getBooksMap(user["bookHistory"]), hover, "bookHistory");
      break;
      case pageType.bought:
        loadHoverMap(currentBooks, hover,);
      break;
      case pageType.rented:
        loadHoverMap(currentBooks, hover,);
      break;
      case pageType.personalBooks:
        loadHover(currentBooks.size, currentBooks, hover, "personalBooks");
      break;
      case pageType.adminBookView:
      break;
      case pageType.recommendation:
        loadHover(getBooksMap(user["recommendedBook"]).size, getBooksMap(user["recommendedBook"]), hover, "recommendedBook");
      break;
      default:
      break;
    }
...
dynamic getBooksList(var bookList, {bool isList = false}) {
  ...
  for(var id in bookList) {
    if(bookMap.keys.contains(id))
      bookData.addLast(bookMap[id]);
  }
...
Future<void> chooseImage(BuildContext context, bool imageSource) async {
  ...
  // File _image;
  dynamic _image;
  final pickedFile = await ImagePicker().getImage(source: imageSource ? ImageSource.camera : ImageSource.gallery);
  if(pickedFile!=null) {
...
String loadBookTooltip(String id) {
  ...
  userBookDetails = isRented(id) ? "Rented on: ${user["booksRented"][id]["rentedOn"]}\Due on: ${user["booksRented"][id]["dueOn"]}\n" : "Rented on: Not Yet\n";
  userBookDetails += "\n";
  userBookDetails += isPurchased(id) ? "Purchased on: ${user["booksBought"][id]["purchasedOn"]}" : "Purchased on: Not Yet";
